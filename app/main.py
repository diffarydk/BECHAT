...
from app.models import Chat, ChatMember, FileItem, FriendRequest, Message, User
        ...
        if chat.type == "group" and chat.last_message and ":" not in chat.last_message:
            ...
            if latest_msg:
                sender_res = await db.execute(
                    select(User).where(User.id == latest_msg.sender_id)
                )
        ...
        chat_sessions.append(
            {
                ...
                "time": format_time_readable(chat.last_message_time)
...
async def get_friend_requests(
    ...
    result = await db.execute(
        select(FriendRequest).where(
            FriendRequest.to_id == current_user.id, FriendRequest.status == "pending"
        )
    )
    ...
    for r in requests:
        ...
        response.append({"id": r.id, "fromId": r.from_id, "fromName": sender_name})
...
async def send_friend_request(
    ...
    await sio.emit(
        ...
        {"id": new_req.id, "fromId": current_user.id, "fromName": current_user.name},
        room=f"user_{to_id}",
    )
...
@app.post(
    ...
    response_model=FriendRequestAcceptResponse,
)
async def accept_friend_request(
    ...
    await sio.emit(
        ...
        {"requestId": request_id, "chat": chat_data_for_sender},
...
@app.post(
    "/api/files", response_model=FileItemResponse, status_code=status.HTTP_201_CREATED
)
...
async def download_file(file_id: str, db: AsyncSession = Depends(get_db)):
    ...
    if not file_item:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail={"message": "File not found"}
        )
...
async def sendMessage(sid, data):
    ...
    async with AsyncSessionLocal() as db:
        ...
        await db.execute(
            ...
            .values(
                last_message=chat_last_msg, last_message_time=datetime.datetime.utcnow()
            )
        )

        # 4. Query all members BEFORE commit so the session is still active
        ...
        # 5. Construct payload for client
        # isSelf is set to False here; frontend determines it by comparing senderId with current user id
        payload_received = {
            "id": msg_id,
            "senderId": sender_id,
            "senderName": sender_name,
            ...
            "image": "",
            "status": "delivered",
            "isSelf": False,  # Frontend determines isSelf by comparing senderId with current user id
        }

        # 6. Broadcast message to all users in chat room
        await sio.emit("receiveMessage", payload_received, room=f"chat_{chat_id}")

        # 7. Notify sidebar updates in real-time
        ...
        for member in members:
            ...
            await sio.emit(
                "chatUpdated", chat_update_payload, room=f"user_{member.user_id}"
            )
